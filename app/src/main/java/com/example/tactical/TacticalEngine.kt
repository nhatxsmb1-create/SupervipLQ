...
class TacticalEngine {
    ...
    private val rosterAnalyzer = AoVRosterAnalyzer()
    ...
    fun evaluateGameState(gameState: GameState, currentState: TacticalState): TacticalEvaluationResult {
        ...
        if (coachStatus == CoachStatus.IN_HERO_SELECTION) {
            val heroState = currentState.copy(
                ...
                dangerWarning = "Ban-Pick: Ưu tiên cấm tướng khống chế cứng (Arum, Aleister) & sát thương cơ động (Aoi, Florentino).",
        ...
        // Perform Deep Roster & Scoreboard Analysis
        val rosterAnalysis = rosterAnalyzer.analyzeRosters(
            allyRoster = gameState.allyRoster,
            enemyRoster = gameState.enemyRoster,
            rawText = gameState.rawOcrSummary
        )
        ...
        // 5. Special Mode Handling (Scoreboard Open or Shop Open)
        if (gameState.screenState == ScreenState.SCOREBOARD_OPEN) {
            val rosterInfo = if (rosterAnalysis.enemyHeroes.isNotEmpty()) {
                "Đội hình địch: ${rosterAnalysis.enemyHeroes.joinToString(", ")}"
            } else "Bảng điểm chi tiết"

            val counterTip = if (rosterAnalysis.counterItems.isNotEmpty()) {
                rosterAnalysis.counterItems.first().reason
            } else "Kiểm tra trang bị và cấp độ của đối thủ"

            dangerWarning = "Bảng Thông Số: $rosterInfo. $counterTip"
            dangerLevel = DangerLevel.MEDIUM
            voiceCallout = rosterAnalysis.tacticalCallout
            calloutTag = "scoreboard_deep_analysis"
            calloutPriority = 3
        } else if (gameState.screenState == ScreenState.SHOP_OPEN) {
            val shopGold = gameState.currentShopGold ?: 0
            if (shopGold >= 2000) {
                dangerWarning = "Kho Đồ: Đang có $shopGold Vàng dư! Chuẩn bị đổi đồ: Giáp Hộ Mệnh ➔ Liềm Đoạt Mệnh / Quả Cầu Băng Sương."
                dangerLevel = DangerLevel.SAFE
                voiceCallout = "Bạn đang có $shopGold vàng dư. Chuẩn bị đổi đồ giữ mạng khi hồi sinh"
                calloutTag = "shop_late_game_swap"
                calloutPriority = 3
            } else {
                dangerWarning = "Kho Đồ: Mua sắm trang bị theo tiến trình."
                dangerLevel = DangerLevel.SAFE
                calloutTag = "shop_general"
            }
        } else {
            // General In-Match Timeline Rules
            when {
                goldDiff <= -5000 -> {
        ...
        val finalItemRecommendations = if (rosterAnalysis.counterItems.isNotEmpty()) {
            rosterAnalysis.counterItems
        } else {
            listOf(
        ...
        val carryTarget = if (rosterAnalysis.threats.isNotEmpty()) {
            val top = rosterAnalysis.threats.first()
            "${top.heroName} (${top.kda})"
        } else if (enemyKills >= 8) {
            "Xạ Thủ / Pháp Sư đối phương (Đang xanh)"
        } else {
            "Chủ Lực Địch"
        }
        ...
        val newState = currentState.copy(
            ...
            topThreats = rosterAnalysis.threats,
            itemRecommendations = finalItemRecommendations
        )
